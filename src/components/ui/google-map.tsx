...
export function GoogleMap({
  ...
  const {
    ...
    error: configError,
  } = trpc.mapConfig.getCurrent.useQuery(undefined, {
    retry: 3,
    retryDelay: 1000,
  });
        ...
        try {
          ...
          const popupContent = `
            <div style="font-family: Arial, sans-serif; padding: 4px;">
              <h3 style="margin: 0 0 8px 0; color: var(--foreground, #1f2937); font-size: 16px;">${
                mapConfig.locationName
              }</h3>
              <p style="margin: 0; color: var(--muted-foreground, #6b7280); font-size: 12px;">
                📍 ${mapConfig.latitude.toFixed(
                  4
                )}, ${mapConfig.longitude.toFixed(4)}
              </p>
            </div>
          `;
      ...
      >
        <div className="text-center p-4">
          ...
          <button
            ...
            className="mt-2 px-3 py-1 bg-destructive text-destructive-foreground text-xs rounded hover:bg-destructive/90 transition-colors"
          >
            Reload Page
          </button>
        </div>
  ...
  return (
    <div
      className={`rounded-lg overflow-hidden shadow-lg border border-gray-200 dark:border-gray-700 ${className} transition-all duration-300`}
    >
      <div
        ref={mapRef}
      ...
      <div className="bg-card p-3 border-t">
        ...
        <p className="text-sm text-muted-foreground">
          📍 Coordinates: {mapConfig.latitude.toFixed(4)},{" "}
          {mapConfig.longitude.toFixed(4)}
        </p>
