...
import { CreditCard, Shield, Calendar, CheckCircle, XCircle, Clock, Euro } from 'lucide-react'
...
export function StripePaymentProcessing({
  ...
  const checkInsuranceStatus = useCallback(async () => {
    try {
      const status = await StripeClientService.checkInsuranceStatus(athlete.id, season.id)
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const validation = StripeClientService.validatePaymentRequest(paymentRequest)
      ...
      if (!result.success) {
        toast.error(result.error || 'Erreur lors de la redirection vers le paiement')
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <p className="font-bold text-xl text-red-600">{formatAmount(INSURANCE_AMOUNT)}</p>
            </div>
          </div>
        </CardContent>
      </Card>
                    ...
                    <>
                      {insuranceStatus.paymentDate && (
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-medium">{insuranceStatus.paymentDate}</span>
                        </div>
                      )}
                      {insuranceStatus.expiryDate && (
                        <div className="flex items-center justify-between">
                          <span>Date d&apos;expiration:</span>
                          <span className="font-medium">{insuranceStatus.expiryDate}</span>
                        </div>
                      )}
                    </>
                  )}
        ...
        <TabsContent value="payment" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Paiement de l&apos;assurance annuelle pour la saison {season.year}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold">{formatAmount(INSURANCE_AMOUNT)}</span>
                </div>
                ...
                <div className="flex justify-between items-center font-bold text-lg">
                  ...
                  <span className="text-red-600">{formatAmount(INSURANCE_AMOUNT)}</span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {insuranceStatus?.hasValidInsurance ? (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    <div className="flex items-center gap-2 text-green-800">
                      ...
                      <span className="font-medium">Assurance déjà active pour cette saison</span>
                    </div>
                    <p className="text-sm text-green-600 mt-1">
                      Votre assurance est valide jusqu&apos;au {insuranceStatus.expiryDate}
                    </p>
                  </div>
                ) : (
                  <Button
                ...
                <div className="text-center space-y-2">
                  <p className="text-sm text-gray-600">
                    Paiement sécurisé par Stripe • Cartes acceptées: Visa, Mastercard
                  </p>
