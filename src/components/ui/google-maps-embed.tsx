...
export function GoogleMapsEmbed({ className, showActions = true }: GoogleMapsEmbedProps) {
  const { data: mapConfig, isLoading: isConfigLoading } = trpc.mapConfig.getCurrent.useQuery()
        ...
        <CardContent className="p-0">
          <div className="flex items-center justify-center h-80 bg-muted/30">
            <div className="flex flex-col items-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground">Aucune configuration de carte trouvée</p>
            </div>
          </div>
        </CardContent>
      </Card>
  ...
  const staticMapUrl = `https://maps.googleapis.com/maps/api/staticmap?center=${latitude},${longitude}&zoom=${mapConfig.zoom || 15}&size=600x400&markers=color:red%7C${latitude},${longitude}&format=png`
  
  // Google Maps direct link for "Open in Maps" button
  const mapsUrl = `https://www.google.com/maps?q=${latitude},${longitude}&z=${mapConfig.zoom || 15}`
          ...
          >
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center text-white">
                ...
                <p className="text-sm opacity-90">Cliquez pour ouvrir Google Maps</p>
              </div>
            </div>
