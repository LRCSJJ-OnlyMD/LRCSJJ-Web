...
export default function ClubManagerPaymentsPage() {
  ...
  const handleCreatePayment = () => {
    ...
    const athleteTab = document.querySelector(
      '[data-value="athletes"]'
    ) as HTMLElement;
    if (athleteTab) {
      athleteTab.click();
      toast.success(
        'Créez des paiements depuis l\'onglet "Gestion des Athlètes"'
      );
    } else {
      toast.info(
        'Utilisez l\'onglet "Gestion des Athlètes" pour créer de nouveaux paiements'
      );
    }
  };
  ...
  const handleGenerateInvoice = (paymentId: string) => {
    ...
    if (payment.status !== "PAID") {
      toast.error(
        "Impossible de générer une facture pour un paiement non effectué"
      );
      return;
    }
    ...
    const generateInvoicePDF = () => {
      ...
      // Create PDF content
      const pdfContent = `
Facture #${invoiceData.paymentId}

Informations client:
Nom: ${invoiceData.athleteName}

Description: ${invoiceData.description}
Montant: ${invoiceData.amount} ${invoiceData.currency}
Date de paiement: ${
        invoiceData.paidAt
          ? new Date(invoiceData.paidAt).toLocaleDateString("fr-FR")
          : "N/A"
      }
Méthode de paiement: ${
...
`.trim();
      ...
      if (link.download !== undefined) {
        ...
        link.setAttribute(
          "download",
          `facture_${payment.id}_${payment.athleteName.replace(
            /\s+/g,
            "_"
          )}.txt`
        );
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground">
              Gérez les paiements, générez des codes de paiement et suivez les
              encaissements
            </p>
          </div>
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>
                  Demandes de Paiement ({filteredPayments.length})
                </CardTitle>
                <CardDescription>
                    ...
                    <TableBody>
                      {filteredPayments.map((payment) => (
                        <TableRow key={payment.id}>
                          ...
                          <TableCell>
                            <div
                              className={`flex items-center gap-2 px-2 py-1 rounded-full text-xs font-medium w-fit ${getStatusColor(
                                payment.status
                              )}`}
                            >
