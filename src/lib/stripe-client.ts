...
export class StripeClientService {
   ...
   */
  static async redirectToCheckout(paymentRequest: ClientPaymentRequest): Promise<{
    success: boolean
    error?: string
  }> {
    try {
  ...
  static async getPaymentHistory(athleteId: string): Promise<{
    ...
    try {
      const response = await fetch(`/api/payments/history?athlete_id=${athleteId}`)
      const data = await response.json()
  ...
  static async checkInsuranceStatus(athleteId: string, seasonId: string): Promise<{
    ...
    try {
      const response = await fetch(`/api/payments/insurance-status?athlete_id=${athleteId}&season_id=${seasonId}`)
      const data = await response.json()
      ...
      return {
        ...
        paymentDate: data.paymentDate ? new Date(data.paymentDate).toLocaleDateString('fr-FR') : undefined,
        expiryDate: data.expiryDate ? new Date(data.expiryDate).toLocaleDateString('fr-FR') : undefined,
